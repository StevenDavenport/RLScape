	...
	public static void main(String[] args) {
		try {
			...
			for (int i = 0; i < args.length; i++) {
				switch(args[i]) {
					case "-headless":
						ClientSettings.HEADLESS = true;
						break;
					case "-rl":
						ClientSettings.RL_BRIDGE_ENABLED = true;
						break;
					case "-dev"	:
					case "-local":
					case "-offline":
				...
				if (args[i].startsWith("-") && (i + 1) < args.length  && !args[i + 1].startsWith("-")) {
					switch(args[i]) {
						case "-rl-port":
							ClientSettings.RL_BRIDGE_PORT = Integer.parseInt(args[++i]);
							break;
						case "-s":
						case "-server":
						case "-ip":
			...
			if (ClientSettings.RL_BRIDGE_ENABLED) {
				RLBridge.start(game, ClientSettings.RL_BRIDGE_PORT);
			}
