...
public class RSApplet extends Applet implements Runnable, MouseListener, MouseWheelListener, MouseMotionListener, KeyListener, FocusListener, WindowListener {
	...
	final void createClientFrame(int i, int j) {
		myWidth = j;
		myHeight = i;
		if (ClientSettings.HEADLESS) {
			headlessImage = new BufferedImage(myWidth, myHeight, BufferedImage.TYPE_INT_RGB);
			headlessPixels = ((DataBufferInt) headlessImage.getRaster().getDataBuffer()).getData();
			headlessGraphics = headlessImage.getGraphics();
		...
		fullGameScreen = new RSImageProducer(myWidth, myHeight, ClientSettings.HEADLESS ? null : getGameComponent());

		if (!ClientSettings.HEADLESS && ClientSettings.SHOW_NAVBAR && !ClientSettings.RL_BRIDGE_ENABLED) {
			try {
	...
	BufferedImage headlessImage;
	int[] headlessPixels;
	private Graphics headlessGraphics;

	public int[] getHeadlessPixels() {
		if (headlessPixels != null) {
			return headlessPixels;
		}
		if (fullGameScreen != null) {
			return fullGameScreen.anIntArray315;
		}
		return null;
	}

	public int getHeadlessWidth() {
		return myWidth;
	}

	public int getHeadlessHeight() {
		return myHeight;
	}

	public synchronized void rlMouseMove(int x, int y) {
		mouseX = x;
		mouseY = y;
	}

	public synchronized void rlMousePress(int button) {
		idleTime = 0;
		clickX = mouseX;
		clickY = mouseY;
		clickTime = System.currentTimeMillis();
	...
	public synchronized void rlMouseRelease(int button) {
		idleTime = 0;
		if (button == 2) {
			mouseWheelDown = false;
		}
