...
final class RSImageProducer implements ImageProducer, ImageObserver {
	...
	public RSImageProducer(int i, int j, Component component) {
		...
		if (ClientSettings.HEADLESS || component == null) {
			BufferedImage image = new BufferedImage(anInt316, anInt317, BufferedImage.TYPE_INT_RGB);
			anIntArray315 = ((DataBufferInt) image.getRaster().getDataBuffer()).getData();
			anImage320 = image;
		} else {
