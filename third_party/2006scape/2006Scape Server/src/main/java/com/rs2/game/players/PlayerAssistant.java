...
public class PlayerAssistant {
	...
	public void firstTimeTutorial() {
		System.out.println("DEBUG: firstTimeTutorial called - TUTORIAL_ISLAND=" + Constants.TUTORIAL_ISLAND + ", tutorialProgress=" + player.tutorialProgress + ", player=" + player.playerName);
		...
		if (Constants.RL_MODE) {
			...
			player.getPlayerAssistant().movePlayer(Constants.RESPAWN_X, Constants.RESPAWN_Y, 0);
		...
		if (Constants.TUTORIAL_ISLAND) {
			System.out.println("DEBUG: Going to Tutorial Island for player " + player.playerName);
			player.getItemAssistant().deleteAllItems();
			player.getPlayerAssistant().hideAllSideBars();
			player.teleportToX = 3094;
			player.teleportToY = 3106;
			movePlayer(3094, 3106, 0);
			...
			sendAutoRetalitate();
			LightSources.saveBrightness(player);
		} else if (!Constants.TUTORIAL_ISLAND) {
			System.out.println("DEBUG: Going to Lumbridge - TUTORIAL_ISLAND=" + Constants.TUTORIAL_ISLAND + ", tutorialProgress=" + player.tutorialProgress);
			player.getPlayerAssistant().sendSidebars();
			PlayerAssistant.removeHintIcon(player);
			player.getPacketSender().walkableInterface(-1);
			player.getPacketSender().chatbox(-1);
