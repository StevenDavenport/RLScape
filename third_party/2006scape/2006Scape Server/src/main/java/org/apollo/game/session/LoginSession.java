...
//		}
		...
		if (PlayerHandler.isPlayerOn(username)) {
			if (Constants.RL_MODE) {
				int existingId = PlayerHandler.getPlayerID(username);
				if (existingId >= 0 && PlayerHandler.players[existingId] != null) {
					PlayerHandler.players[existingId].disconnected = true;
				}
			} else {
				returnCode = 5;
			}
